...
##UNA DISTRIBUZIONE DIVERSA PER OGNI MESE

setwd("C:\\Users\\Benedetta\\OneDrive - Politecnico di Torino\\Documenti\\SSI\\Crissolo Po")

# Leggi i dati dal file CSV
my_df <- read_csv("CRISSOLO PO.csv")

# Converte la colonna della data in formato data
my_df$DATA <- as.Date(my_df$DATA, format="%Y-%m-%d")

my_df$Anno <- format(my_df$DATA, "%Y")
my_df$Mese <- format(my_df$DATA, "%m")

# Inizializza un dataframe vuoto per memorizzare i dati delle precipitazioni cumulative
...
for (anno in unique(my_df$Anno)) {
  for (mese in 1:12) {
    ...
    # Calcola la media della portata fiume per il mese corrente sostituendo i valori mancanti con la media del mese
    media_mese <- mean(dati_mese_anno$"Portata fiume (m³/s)", na.rm = TRUE)
    dati_mese_anno$"Portata fiume (m³/s)"[is.na(dati_mese_anno$"Portata fiume (m³/s)")] <- media_mese
    
    # Calcola di nuovo la media della portata fiume per il mese corrente con i valori sostituiti
    media_mese <- mean(dati_mese_anno$"Portata fiume (m³/s)", na.rm = TRUE)
...
ggplot(dati_portate_medie, aes(x = Year_Month, y = portata_media)) +
  ...
  labs(title = "Portate Medie Mensili a POCS",
       x = "",
       y = "Portata Media (m³/s)") +
  theme_minimal() +
...
# Visualizza la lista delle migliori distribuzioni per ogni mese
print(migliori_distribuzioni)
...
# Esegui il plot per ogni colonna di portate_mensili
grafici <- list()
for (col in names(portate_mensili)[-1]) {
  data <- portate_mensili[[col]][complete.cases(portate_mensili[[col]])]
  ...
  grafico <- plot_cdf(data, distribuzione, parametri, titolo)
  if (!is.null(grafico)) {
    grafici[[col]] <- grafico
  }
}

# Combina i grafici in una griglia di 3 colonne
do.call(grid.arrange, c(grafici, ncol = 3))
...
write.xlsx(distribuzione_ogni_mese, "SSI_Crissolo Po.xlsx")
...
grafico_ssi <- ggplot(distribuzione_ogni_mese, aes(x = Date, y = SSI, fill = Colore_ssi)) +
  ...
  labs(x = "", y = "", title = "SSI indice a POCS") +
  theme_minimal() +
  theme(
...
file_path <- "C:/Users/Benedetta/OneDrive - Politecnico di Torino/Documenti/SSI/Grafici SSI/POCS.png"
...
write.xlsx(eventi_siccitosi, "eventi Crissolo Po.xlsx")
...
# evento_in_corso <- FALSE
# for (i in 1:nrow(distribuzione_ogni_mese)) {
#   if (!is.na(distribuzione_ogni_mese$SSI[i]) && !evento_in_corso && distribuzione_ogni_mese$SSI[i] < -1) {
#     evento_in_corso <- TRUE
#     distribuzione_ogni_mese$Evento_siccita_ssi[i] <- "Siccita in corso SSI"
#   } else if (evento_in_corso && !is.na(distribuzione_ogni_mese$SSI[i]) && distribuzione_ogni_mese$SSI[i] < 0) {
#     distribuzione_ogni_mese$Evento_siccita_ssi[i] <- "Siccita in corso SSI"
#   } else if (evento_in_corso && !is.na(distribuzione_ogni_mese$SSI[i]) && distribuzione_ogni_mese$SSI[i] >= 0) {
#     evento_in_corso <- FALSE
#     distribuzione_ogni_mese$Evento_siccita_ssi[i] <- "Fine siccita SSI"
#   }
# }
# 
# 
# # Riempimento del colore in base agli eventi di siccità
# distribuzione_ogni_mese$Colore_ssi <- ifelse(distribuzione_ogni_mese$Evento_siccita_ssi == "Siccita in corso SSI", "Rosso", "Nero")
# 
# # Grafico dell'indice SSI con colori per gli eventi di siccità
# ggplot(distribuzione_ogni_mese, aes(x = Year + Month / 12, y = SSI, fill = Colore_ssi)) +
#   geom_bar(stat = "identity", width = 0.08) +
#   scale_fill_manual(values = c("Rosso" = "red", "Nero" = "black"), labels = c("Rosso" = "Drought Runs", "Nero" = "SSI")) +
#   geom_hline(yintercept = c(-1), linetype = "dashed", color = "red", size = 0.5) +
#   geom_hline(yintercept = c(1), linetype = "dashed", color = "blue", size = 0.5) +
#   labs(x = "", y = "", title = "SSI a a Polonghera con distribuzione fittata per ogni mese") +
#   theme_minimal() +
#   theme(axis.text.x = element_text(angle = 45, hjust = 1),
#         legend.position = c(0.93, 0.95),
#         legend.title = element_blank(),  # Rimuove il titolo della legenda
...
#         axis.line = element_line(color = "black"),
#         legend.box.background = element_rect(color = "black", size = 0.5)) +  # Aggiunge il riquadro attorno alla legenda
#   scale_x_continuous(expand = c(0, 0), breaks = seq(min(distribuzione_ogni_mese$Year), max(distribuzione_ogni_mese$Year), by = 1))
# 
...
# ######UNA DISTRIBUZIONE PER TUTTA LA STAZIONE 
# setwd("C:\\Users\\Benedetta\\OneDrive - Politecnico di Torino\\Documenti\\SSI\\Rossana Varaita")
# 
# # Leggi i dati dal file CSV
# my_df <- read_csv("ROSSANA VARAITA.csv")
# 
# # Converte la colonna della data in formato data
# my_df$DATA <- as.Date(my_df$DATA, format="%Y-%m-%d")
# 
...
# library(tidyverse)
# 
# # Utilizza la funzione pivot_wider per trasformare il dataframe
# portate_mensili <- dati_portate_medie %>%
#   pivot_wider(names_from = Month, values_from = portata_media)
# 
# # Visualizza il nuovo dataframe
# print(portate_mensili)
# 
# # Stampiamo il DataFrame
# print(dati_portate_medie)
# 
# # Carica il pacchetto fitdistrplus
# library(fitdistrplus)
# library(MASS)
# library(actuar)
# library(evd)
# # Filtra le righe con valori NA nella colonna portata_media
# dati_portate_medie <- dati_portate_medie[complete.cases(dati_portate_medie$portata_media), ]
# 
# # Fit delle distribuzioni ai dati
# fit_ln <- fitdist(dati_portate_medie$portata_media, "lnorm")
# fit_weibull <- fitdist(dati_portate_medie$portata_media, "weibull")
# fit_gamma <- fitdist(dati_portate_medie$portata_media, "gamma")
# fit_gev <- fitdist(dati_portate_medie$portata_media, "gev", start = list(loc = 0, scale = 1, shape = 0))
# fit_pp <- fitdist(dati_portate_medie$portata_media, "pareto")
# fit_ll <- fitdist(dati_portate_medie$portata_media, "llogis")
# 
# plot.legend <- c( "lognormal", "Weibull","gamma", "GEV", "pareto", "log-logistic")
# qqcomp(list(fit_ln, fit_weibull, fit_gamma, fit_gev, fit_pp,fit_ll), legendtext = plot.legend)
# denscomp(list(fit_ln, fit_weibull, fit_gamma, fit_gev, fit_pp,fit_ll), legendtext = plot.legend)
# cdfcomp(list(fit_ln, fit_weibull, fit_gamma, fit_gev, fit_pp,fit_ll), legendtext = plot.legend)
# ppcomp(list(fit_ln, fit_weibull, fit_gamma, fit_gev, fit_pp,fit_ll), legendtext = plot.legend)
# 
# # Calcola l'AIC per ogni distribuzione
# aic_ln <- fit_ln$aic
# print(paste("Log-Normal AIC:", aic_ln))
# aic_weibull <- fit_weibull$aic
# print(paste("Weibull AIC:", aic_weibull))
# aic_gamma <- fit_gamma$aic
# print(paste("Gamma AIC:", aic_gamma))
# aic_gev <- fit_gev$aic
# print(paste("GEV AIC:", aic_gev))
# aic_pareto <- fit_pp$aic
# print(paste("Pareto AIC:", aic_pareto))
# aic_ll<-fit_ll$aic
# print(paste("LOG-LOGISTIC:", aic_ll))
# 
# # Calcola l'AIC per ogni distribuzione
# aic_values <- c(aic_ln, aic_weibull, aic_gamma, aic_gev, aic_pareto, aic_ll)
# 
# # Trova l'indice del minimo valore di AIC
# best_fit_index <- which.min(aic_values)
# 
# # Seleziona la distribuzione migliore in base all'indice trovato
# best_distribution <- switch(best_fit_index,
#                             "1" = "lnorm",
#                             "2" = "weibull",
#                             "3" = "gamma",
#                             "4" = "gev",
...
#                             "6" = "llogis")
# 
# # Salva i parametri della distribuzione migliore
# best_parameters <- switch(best_fit_index,
#                           "1" = fit_ln$estimate,
#                           "2" = fit_weibull$estimate,
#                           "3" = fit_gamma$estimate,
#                           "4" = fit_gev$estimate,
#                           "5" = fit_pp$estimate,
#                           "6" = fit_ll$estimate)
# 
# # Crea un dataframe per salvare i risultati
# best_fit_data <- data.frame(Distribution = best_distribution, Parameters = best_parameters)
# 
# # Stampiamo i risultati
# print("Distribuzione migliore e relativi parametri:")
# print(best_fit_data)
# 
# 
# # Seleziona i parametri della distribuzione log-logistica dalla tua analisi
# shape <- best_parameters[1]  # Forma
# scale <- best_parameters[2]  # Scala
# 
# # Calcola la probabilità cumulata con la distribuzione log-logistica
# prob_cumulative <- pllogis(dati_portate_medie$portata_media, shape = shape, scale = scale)
# 
# # Stampiamo i risultati
# print("Probabilità cumulata con distribuzione log-logistica:")
# print(prob_cumulative)
# 
# # Normalizza la distribuzione cumulata utilizzando la funzione qnorm
# standardized_values <- qnorm(prob_cumulative, mean = 0, sd = 1)
# 
# # Stampiamo i risultati
# print("Distribuzione standard:")
# print(standardized_values)
# 
# 
# # Aggiungi la colonna con l'indice SSI al dataframe dati_portate_medie
# dati_portate_medie$SSI_Polonghera <- standardized_values
# 
# write.xlsx(dati_portate_medie, "SSI_Polonghera.xlsx")
# 
# # Visualizza il dataframe aggiornato
# print("Dati con l'indice SSI:")
# print(dati_portate_medie)
# 
# 
# library(ggplot2)
# 
# 
# # Inizializzazione della colonna degli eventi di siccità
# dati_portate_medie$Evento_siccita_ssi <- ""
# 
# # Trova gli eventi di siccità SSI
# evento_in_corso <- FALSE
# for (i in 1:nrow(dati_portate_medie)) {
#   if (!is.na(dati_portate_medie$SSI_Polonghera[i]) && dati_portate_medie$SSI_Polonghera[i] < -1) {
#     evento_in_corso <- TRUE
#     dati_portate_medie$Evento_siccita_ssi[i] <- "Siccita in corso SSI"
#   } else if (evento_in_corso && dati_portate_medie$SSI_Polonghera[i] < 0) {
#     dati_portate_medie$Evento_siccita_ssi[i] <- "Siccita in corso SSI"
#   } else if (evento_in_corso && dati_portate_medie$SSI_Polonghera[i] >= 0) {
#     evento_in_corso <- FALSE
#     dati_portate_medie$Evento_siccita_ssi[i] <- "Fine siccita SSI"
#   }
# }
# 
# # Riempimento del colore in base agli eventi di siccità
# dati_portate_medie$Colore_ssi <- ifelse(dati_portate_medie$Evento_siccita_ssi == "Siccita in corso SSI", "Rosso", "Nero")
# 
# dati_portate_medie$Year <- as.numeric(dati_portate_medie$Year)
# dati_portate_medie$Month <- as.numeric(dati_portate_medie$Month)
# 
# 
# # Grafico dell'indice SSI con colori per gli eventi di siccità
# ggplot(dati_portate_medie, aes(x = Year + Month / 12, y = SSI_Polonghera, fill = Colore_ssi)) +
#   geom_bar(stat = "identity", width = 0.08) +
#   scale_fill_manual(values = c("Rosso" = "red", "Nero" = "black"), labels = c("Rosso" = "Drought Runs", "Nero" = "SSI")) +
#   geom_hline(yintercept = c(-1), linetype = "dashed", color = "red", size = 0.5) +
#   geom_hline(yintercept = c(1), linetype = "dashed", color = "blue", size = 0.5) +
#   labs(x = "", y = "", title = "SSI a Polonghera co una distribuzione per tutta la stazione") +
#   theme_minimal() +
#   theme(axis.text.x = element_text(angle = 45, hjust = 1),
#         legend.position = c(0.93, 0.95),
#         legend.title = element_blank(),  # Rimuove il titolo della legenda
...
#         axis.line = element_line(color = "black"),
#         legend.box.background = element_rect(color = "black", size = 0.5)) +  # Aggiunge il riquadro attorno alla legenda
#   scale_x_continuous(expand = c(0, 0), breaks = seq(2000, max(dati_portate_medie$Year), by = 5))
# 
# 
# 
# # Grafico dell'indice SSI con colori per gli eventi di siccità
...
#   geom_bar(stat = "identity", width = 0.08) +
#   scale_fill_manual(values = c("Rosso" = "red", "Nero" = "black"), labels = c("Rosso" = "Drought Runs", "Nero" = "SSI")) +
#   geom_hline(yintercept = c(-1), linetype = "solid", color = "red", size = 0.5) +
#   geom_hline(yintercept = c(1), linetype = "solid", color = "blue", size = 0.5) +
#   labs(x = "", y = "", title = "") +
#   theme_minimal() +
