...
setwd("C:\\Users\\Benedetta\\OneDrive - Politecnico di Torino\\Documenti\\SSI\\Vinadio Stura di Demonte")

my_df <- read_csv("VINADIO STURA DI DEMONTE.csv")
...
write.xlsx(eventi_siccitosi, "eventi Vinadio Stura di Demonte.xlsx")
...
# 
# 
# ######UNA DISTRIBUZIONE PER TUTTA LA STAZIONE DI DRONERO#########
# 
# 
...
# 
# 
# plot.legend <- c( "lognormal", "Weibull","gamma", "pareto", "log-logistic")
# qqcomp(list(fit_ln, fit_weibull, fit_gamma, fit_pp,fit_ll), legendtext = plot.legend)
# denscomp(list(fit_ln, fit_weibull, fit_gamma, fit_pp,fit_ll), legendtext = plot.legend)
# cdfcomp(list(fit_ln, fit_weibull, fit_gamma, fit_pp,fit_ll), legendtext = plot.legend)
# ppcomp(list(fit_ln, fit_weibull, fit_gamma, fit_pp,fit_ll), legendtext = plot.legend)
# 
...
# aic_pp <- fit_pp$aic
# print(paste("Pareto AIC:", aic_pp))
# aic_ll<-fit_ll$aic
# print(paste("LOG-LOGISTIC:", aic_ll))
# 
# # Calcola l'AIC per ogni distribuzione
# aic_values <- c(aic_ln, aic_weibull, aic_gamma, aic_pp, aic_ll)
# 
...
# meanlog <- best_parameters[1] 
# sdlog <- best_parameters[2]  
# 
# # Calcola la probabilità cumulata con la distribuzione log-logistica
# prob_cumulative <- plnorm(dati_portate_medie$portata_media,meanlog = meanlog, sdlog =sdlog)
# 
# # Stampiamo i risultati
# print("Probabilità cumulata con distribuzione lnorm:")
# print(prob_cumulative)
# 
...
# dati_portate_medie$SSI_VinadioDemonte <- standardized_values
# 
# 
# 
# # Visualizza il dataframe aggiornato
...
#   if (!is.na(dati_portate_medie$SSI_VinadioDemonte[i]) && dati_portate_medie$SSI_VinadioDemonte[i] < -1) {
#     evento_in_corso <- TRUE
#     dati_portate_medie$Evento_siccita_ssi[i] <- "Siccita in corso SSI"
#   } else if (evento_in_corso && !is.na(dati_portate_medie$SSI_VinadioDemonte[i]) && dati_portate_medie$SSI_VinadioDemonte[i] < 0) {
#     dati_portate_medie$Evento_siccita_ssi[i] <- "Siccita in corso SSI"
#   } else if (evento_in_corso && (is.na(dati_portate_medie$SSI_VinadioDemonte[i]) || dati_portate_medie$SSI_VinadioDemonte[i] >= 0)) {
#     evento_in_corso <- FALSE
...
# 
# # Grafico dell'indice SSI con colori per gli eventi di siccità
# ggplot(dati_portate_medie, aes(x = Year + Month / 12, y = SSI_VinadioDemonte, fill = Colore_ssi)) +
#   geom_bar(stat = "identity", width = 0.08) +
#   scale_fill_manual(values = c("Rosso" = "red", "Nero" = "black"), labels = c("Rosso" = "Drought Runs", "Nero" = "SSI")) +
#   geom_hline(yintercept = c(-1), linetype = "dashed", color = "red", size = 0.5) +
#   geom_hline(yintercept = c(1), linetype = "dashed", color = "blue", size = 0.5) +
#   labs(x = "", y = "", title = "SSI a Vinadio di Demonte co una distribuzione per tutta la stazione") +
#   theme_minimal() +
#   theme(axis.text.x = element_text(angle = 45, hjust = 1),
#         legend.position = c(0.93, 0.95),
#         legend.title = element_blank(),  # Rimuove il titolo della legenda
...
# 
# # Grafico dell'indice SSI con colori per gli eventi di siccità
# ggplot(dati_portate_medie, aes(x = Year + Month / 12, y = SSI_DroneroMaira, fill = Colore_ssi)) +
#   geom_bar(stat = "identity", width = 0.08) +
#   scale_fill_manual(values = c("Rosso" = "red", "Nero" = "black"), labels = c("Rosso" = "Drought Runs", "Nero" = "SSI")) +
...
#   geom_hline(yintercept = c(-1), linetype = "dashed", color = "red", size = 0.5) +
#   geom_hline(yintercept = c(1), linetype = "dashed", color = "blue", size = 0.5) +
#   labs(x = "", y = "", title = "SSI a a Dronero Maira con distribuzione fittata per ogni mese") +
#   theme_minimal() +
#   theme(axis.text.x = element_text(angle = 45, hjust = 1),
#         legend.position = c(0.93, 0.95),
#         legend.title = element_blank(),  # Rimuove il titolo della legenda
